...
def validate(run_manager, epoch=0, is_test=False, image_size_list=None,
             ks_list=None, expand_ratio_list=None, depth_list=None, scale_list=None, width_mult_list=None,
    ...
    for setting, name in subnet_settings:
        ...
        loss, (epe, d1, thres1, thres2, thres3) = run_manager.validate(epoch=epoch, is_test=is_test, run_str=name,
                                                                       net=dynamic_net)
    ...
    with tqdm(total=nBatch,
        ...
        for i, sample in enumerate(run_manager.run_config.train_loader):
            ...
            t.set_postfix({
                ...
                **run_manager.get_metricvals(metric_dict, return_dict=True),
                'R': left.size(2),
                'lr': new_lr,
                'loss_type': loss_type,
...
def train_elastic_scale(train_func, run_manager, args, validate_func_dict):
    ...
    if len(set(dynamic_net.ks_list)) == 1 and len(set(dynamic_net.depth_list)) == 1 and len(
