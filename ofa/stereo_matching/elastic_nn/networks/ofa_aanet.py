...
class OFAAANet(AANet):
    ...
    def __init__(self, bn_param=(0.1, 1e-5), base_stage_width=None, width_mult=1.0,
                 ks_list=3, expand_ratio_list=6, depth_list=4, scale_list=4):
        ...
        super(OFAAANet, self).__init__(max_disp=192, feature_blocks=self.feature_blocks,
                                       num_scales=max(self.scale_list))
        # runtime_depth

        self.runtime_depth = [len(block_idx) for block_idx in self.fea_block_group_info]  # 运行时块个数
    ...
    def feature_extraction(self, img):
        ...
        for stage_id, block_idx in enumerate(self.fea_block_group_info):
            if stage_id < self.active_scale:  # 根据active_scale选取前几个特征阶段
                ...
                features.append(x)
        return features  # 返回特征大小不同的特征的列表
